...
# Import sample points (created using sptial random sampling in ArcGIS).
...
# Extract access data with sample points.
city_access <- rast("G:/data/Accessibility to cities/accessibility_to_cities_2015_v1.0/accessibility_to_cities_2015_v1.0.tif")
road_access <- rast("G:/data/road/road22.tif") %>% 
  project(., crs = crs(city_access))

# Access to cities nearby.
city_access <- ifel(city_access == -9999, NA, city_access) %>% 
  ifel(. < 0, 0, .)
city_access_list <- list()
for (samp_name in names(samps))
{
  city_access_list[[samp_name]] <- extract(city_access, samps[[samp_name]], buffer = 2000, fun = mean, na.rm = T, ID = F)[, 1]
}
city_access_df <- do.call(cbind, city_access_list)
colnames(city_access_df) <- c("07","10","13","16","19")
write.csv(city_access_df, "./data/city_access.csv", row.names = F)

# Access to roads nearby.
road_access <- ifel(road_access == -9999, NA, road_access) %>% 
  ifel(. < 0, 0, .)
road_access_list <- list()
for (samp_name in names(samps))
{
  road_access_list[[samp_name]] <- extract(road_access, samps[[samp_name]], buffer = 2000, fun = mean, na.rm = T, ID = F)[, 1]
}
road_access_df <- do.call(cbind, road_access_list)
colnames(road_access_df) <- c("07","10","13","16","19")
write.csv(road_access_df, "./data/road_access.csv", row.names = F)

# Import human footprint data and extract with sample points.
file_names <- c("human05_07", "human08_10", "human11_13", "human14_16", "human17_19")
...
city_access = read.csv("./data/city_access.csv")
road_access = read.csv("./data/road_access.csv")
...
Data$city = c(city_access$X07,city_access$X10,city_access$X13,city_access$X16,city_access$X19)
Data$road =  c(road_access$X07,road_access$X10,road_access$X13,road_access$X16,road_access$X19)
