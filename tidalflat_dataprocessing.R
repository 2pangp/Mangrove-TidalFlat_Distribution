...
CityAccess = rast("E:/tidalflat/focalresult/cityaccessibility.tif")
RoadAccess = rast("G:/data/road/roadaccessibility.tif")
RoadAccess = project(RoadAccess, crs(CityAccess))

CityAccess = focal(CityAccess, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
...
human07 = focal(human07, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
human10 = focal(human10, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
human13 = focal(human13, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
human16 = focal(human16, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
human19 = focal(human19, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
...
Pop07 = focal(Pop07, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
Pop10 = focal(Pop10, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
Pop13 = focal(Pop13, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
Pop16 = focal(Pop16, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
Pop19 = focal(Pop19, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
...
GDP10 = focal(GDP10, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
GDP13 = focal(GDP13, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
GDP16 = focal(GDP16, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
GDP19 = focal(GDP19, w=matrix(1,31,31), fun= sum,na.rm=T, pad=T, padValue=NA) 
...
#05Combining data and create a data frame
...
#06Get latitude and longitude data
...
#11Get the data of River and coastline accessibility
river = rast("E:/NUS/new data/river.tif")
coastline = rast("E:/NUS/new data/coastline.tif")
...
write.csv(Data,"./data/tidalflat.csv", row.names = F)
...
library(gbm)
library(caret)
tidalflat = read.csv("./data/tidalflat.csv")
colnames(tidalflat)
...
write.csv(pdp_loss_prev, "./data/wheat.csv", row.names = F)
...
names(pred_list) = c("gdp", "hf", "pop", "city", "road", "PA", "coconut", "coffee", "olp", "rice", 
                     "rubber", "sugarcane","loss_prev","river","coastline","barley","maize",
                     "rapeseed","sorghum","soybean","sunflower","wheat")
writeRaster(pred_list, "./data/23000ssp1pred_list.tif",overwrite =T)


pred_set = rast("./data/2030ssp1pred_list.tif")
PredRas = predict(pred_set,RFmodel,na.rm=T)
writeRaster(PredRas, "E:/tidalflat/inputdata/2030ssp1pred.tif",overwrite =T)
